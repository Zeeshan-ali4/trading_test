...
function frame(svg,values,kind){{const width=Math.max(360,svg.clientWidth||500),height=270,pad={{left:56,right:12,top:14,bottom:34}};svg.setAttribute('viewBox',`0 0 ${{width}} ${{height}}`);svg.replaceChildren();if(!values.length){{svg.append(element('text',{{x:width/2,y:height/2,'text-anchor':'middle',class:'label'}})).textContent='No data';return null}}const ys=values.map(value=>value[1]),min=Math.min(0,...ys),max=Math.max(0,...ys),spread=max-min||1;const x=index=>pad.left+index*(width-pad.left-pad.right)/Math.max(values.length-1,1),y=value=>height-pad.bottom-(value-min)/spread*(height-pad.top-pad.bottom);for(let i=0;i<4;i++){{const yy=pad.top+i*(height-pad.top-pad.bottom)/3;svg.append(element('line',{{x1:pad.left,x2:width-pad.right,y1:yy,y2:yy,class:'grid'}}));const label=svg.append(element('text',{{x:pad.left-6,y:yy+4,'text-anchor':'end',class:'label'}}));label.textContent=`${{((max-i*spread/3)*100).toFixed(1)}}%`}}svg.append(element('line',{{x1:pad.left,x2:width-pad.right,y1:height-pad.bottom,y2:height-pad.bottom,class:'axis'}}));svg.append(element('line',{{x1:pad.left,x2:pad.left,y1:pad.top,y2:height-pad.bottom,class:'axis'}}));return {{width,height,pad,x,y,min,max}}}}
